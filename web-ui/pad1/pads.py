...
f = open("users.txt", "rb")
user_list = f.read().decode('utf-8').split("^")
f.close()
...
if (cur_user != "Guest" and cur_user != ""):
    for u in user_list:
        ...
        if u_array[0] == cur_user:
            ...
            break
s.write("7\r\n".encode('utf-8'))
cur_pressures = s.read(78).decode('utf-8').split(',')
s.close()

f = open("pad1/indexbottom.html", "rb")
print(f.read().decode('utf-8') % (cur_user, int(cur_pressures[3]), int(cur_pressures[1]), int(cur_pressures[5]), int(cur_pressures[7]), cur_user, cur_user))
f.close()
